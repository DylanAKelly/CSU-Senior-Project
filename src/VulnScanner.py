...
def get_app_version(app_name):
    ...
    for package in packages:
        if app_name.lower() in package['Name'].lower():
            ...
            isFound = True
            result = f"{package['Name']} - Version: {package['Version']}"
    
    if not isFound:
        f.write(app_name + " is not installed." + '\n')
        result = f"{app_name} is not installed."
...
def get_vulnSumm(url):
    ...
    if response.status_code == 200:
        ...
        for summary in cve_summaries:
            f.write(summary.get_text(strip=True) + '\n') #strip=True removes leading/trailing whitespaces
        f.close()
...
def user_email():
    ...
    # Verifies that the user has an email saved
    ...
    messagebox.showinfo("Email Updated", "Your email has been changed to: " 
        + userEmail + ". You may now press the 'Begin Scan' button to start the scan.")
...
# Enables the scan button once email is entered
def enable_scan_button():
    scan.config(state=tk.NORMAL)

# Disables the scan button if email is not present or file does not exist
def disable_scan_button():
    scan.config(state=tk.DISABLED)

# Verifies that user email is present before permitting scan
def check_email():
    emailCheck = 'VulSumUserEmail.txt'
    
    try:
        fSize = os.path.getsize(emailCheck)
...
scan = tk.Button(root, text="Begin Scan", bg="lightgreen", command=threading.Thread(target=run_scanner, daemon=True).start, state=tk.DISABLED)
